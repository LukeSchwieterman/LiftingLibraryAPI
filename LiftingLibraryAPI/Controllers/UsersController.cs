...
namespace LiftingLibraryAPI.Controllers
{
	...
	public class UsersController : ControllerBase
	{
        ...
        // POST Login to authenticate the user and generate a JWT
        [HttpPost("login")]
		...
		public async Task<ActionResult<UserContext>> AddUser([FromBody] RegisterUser registerUser)
		{
			try
			{
				...
				return CreatedAtAction(nameof(GetUser), new { id = user.UserId }, new UserContext(user)); // Return the added user data as a UserContext
			}
			catch (Exception ex)
			{
				// Log the exception with user email (but not the password)
        ...
        // DELETE user by ID (Requires authorization)
        [Authorize]
		[HttpDelete("{id}")]
        ...
        // PUT to update user details (Requires authorization)
        [Authorize]
		[HttpPut]
